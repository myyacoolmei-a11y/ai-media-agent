import { threadsGraphBase } from "@/lib/social/config";
...
export async function threadsRequest<T>(
  ...
  const url = new URL(
    `${threadsGraphBase()}${path.startsWith("/") ? path : `/${path}`}`,
  );
  ...
  if (!response.ok || body.error) {
    throw new Error(body.error?.message || `Threads API ${response.status}`);
  }
  return body;
}
